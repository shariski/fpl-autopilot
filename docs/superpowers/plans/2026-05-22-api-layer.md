...
- [x] **Step 1: Add deps to `pyproject.toml`.** Update the dependencies and packages lines:
...
- [x] **Step 2: Add deps to `requirements.txt`** (append):
...
- [x] **Step 3: Create the package + dependency**
...
- [x] **Step 4: Reinstall and verify imports**
...
- [x] **Step 1: Write the failing tests** in `tests/test_api.py` (includes the shared seed helper used by Task 3 too)
...
- [x] **Step 3: Write `src/interface/queries.py`**
...
- [x] **Step 1: Write the failing tests** — append to `tests/test_api.py`
...
- [x] **Step 3: Write `src/interface/api.py`**
...
- [x] **Step 1: Add the `serve` function to `src/cli.py`.** After the `refresh(...)` function (before `main`), add:
...
- [x] **Step 2: Register the subcommand in `main`.** In `main(...)`, after the `refresh` subparser block and before `args = parser.parse_args(argv)`, add:
...
- [x] **Step 3: Verify `--help` lists `serve` and the suite is green**
...
- [x] **Step 1: Populate the DB (data + analytics)**
...
- [x] **Step 2: Launch the server in the background and curl the endpoints**
