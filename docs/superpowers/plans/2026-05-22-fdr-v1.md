...
- [x] **Step 1: Add the v1 subsection.** In `docs/decision-engine.md`, find this line in the `## Fixture Difficulty Rating (custom)` section:
...
- [x] **Step 2: Mark the existing xG formula as v2.** Find this line:
...
- [x] **Step 3: Add a changelog row.** In the `## Changelog (this document)` table at the bottom, after the `v0.1` row, add:

```
| v0.2 | 2026-05-22 | FDR versioned: v1 = FPL-strength quintile (implemented); v2 = xG-based (deferred, team xG unavailable). |
```

- [x] **Step 4: Verify the edits**
...
- [x] **Step 1: Register the new package.** In `pyproject.toml`, change the packages line:
...
- [x] **Step 2: Write the failing tests** in `tests/test_fdr.py`
...
- [x] **Step 4: Write `src/analytics/fdr.py`**
...
- [x] **Step 1: Write the failing tests** — append to `tests/test_fdr.py`
...
- [x] **Step 3: Append `upsert_fdr` to `src/data/repository.py`**
...
- [x] **Step 1: Refresh data and run FDR**
...
- [x] **Step 2: Confirm correctness is proven by tests regardless**
