...
| v0.26 | 2026-09-03 | Cohort-formation rebalance — **prior-GW fallback**. FPL's top-100 leaders don't publish their picks until each GW settles, so `leader_gw_picks` for the upcoming GW is always empty before the Friday deadline. v0.25 no-oped in that window. v0.26 extends `_cohort_formation` to walk back through *finished* GWs (most recent first, up to `MAX_PRIOR_AGE_GWS = 3`) when the upcoming GW has no picks. Rules: (1) upcoming-GW picks still win when present (no aggregation with priors — one source per query); (2) the fallback GW must be `finished=1`; (3) the prior is rejected if it's more than 3 GWs behind (formation trends shift with DGW rotations and fixture swings — older priors are noise); (4) the prior's cohort must still clear `COHORT_FORMATION_MIN = 20` AND its modal must beat the runner-up by more than `MODAL_TIE_MARGIN = 1` — same gates as v0.25. The activity-log entry records the source GW so the prior is auditable (B10). When no GW within the window qualifies, behave exactly as v0.17. The rebalancer, ranker, captain logic and all thresholds are unchanged — this is purely a data-source selection rule.
