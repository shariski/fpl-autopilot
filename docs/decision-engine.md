...
| v0.25 | 2026-09-03 | `execute-lineup` adds **cohort-formation rebalance** (slot swap, no transfers). After the captain/vice/bench-order pass, look up the modal formation from `leader_gw_picks.formation` for the **most-recent finished GW** (the only data point actually fresh enough to plan with — the upcoming GW is always empty pre-deadline, and older priors are stale). If `cohort >= COHORT_FORMATION_MIN = 10` AND the modal differs from the user's current XI shape AND the squad can fill it, swap the lowest-xP starter of the surplus position with the highest-xP bench player of the deficit position (slot 12 is the bench GK anchor — never swapped; only 13/14/15). Captain/vice stay with their player id. `MODAL_TIE_MARGIN = 1` rejects ambiguous modals (top beats runner-up by ≤ 1 vote). Activity-log entry captures the source GW + swap diff (B10). Fallback (no finished GW yet, cohort below 10, modal tie, illegal shape, would bench captain/vice): behave exactly as v0.17. Captain/vice/bench-order logic, the ranker, and all thresholds are unchanged — this is a pure slot-assignment layer that reads existing analytics output.
| v0.26 | 2026-09-03 | `execute-lineup` adds **optimal-XI selection** — pick the 10 starters with the highest next-GW xP from the user's existing 15, subject to FPL's valid formation rules (1 GK + 3-5 DEF + 3-5 MID + 1-3 FWD = 7 valid formations). Captain/vice are chosen from the 10 starters by next-GW xP (desc); the captain must start (if a captain candidate is dropped by the optimizer, force them in and drop the lowest-xP non-captain/non-vice starter instead). Bench order = the 5 leftover players, highest xP first, anchored at slot 12 (sub GK). Reads `xp_v2` only — no new model. New module `src/decisions/optimal_xi.py`; `run_lineup` calls it BEFORE the v0.25 formation rebalancer so the rebalancer sees the chosen starters. When the squad cannot form any valid XI (e.g., fewer than 10 outfield players) the optimizer returns `None` and the caller behaves as before (current captain/vice + bench order + v0.25 rebalance). The transfer engine is untouched — no transfers, no budget changes, no chip decisions (B3). Captain/vice/bench/ranker logic and all thresholds are unchanged.
| v0.28 | 2026-09-04 | `execute-lineup` optimal-XI captain fix (aligns with the ranker; no new model, no threshold edits). The v0.26 optimal-XI optimizer chose captain/vice by **raw next-GW xP** among the chosen 11 starters, bypassing the v0.20 ceiling term + v0.21 pre-season defensive penalty that the captain ranker applies. Live evidence (GW3 26/27): a stale snapshot left Virgil (DEF, xP 6.62) as the top-xP starter and the optimizer made him captain over Mbeumo (MID, xP 6.85) — the exact low-ceiling defensive-captain case the ranker penalty exists to prevent. **Change:** `optimal_xi.select` now scores captain/vice candidates with the same `captain._score` used by the ranker — `xP + 0.15 × (xgoals + xassists)`, minus `PRE_SEASON_DEF_PENALTY = 1.5` for GKP/DEF while the SF window has < 3 live pairs (`ratings.sf_live_pairs` < `SF_LIVE_MIN`). Constants are imported from `src/decisions/captain.py` (single source of truth — no duplicated thresholds). The starting-XI and bench selection remain pure next-GW xP (unchanged); only the armband/vice assignment uses the adjusted score. Captain/vice are still guaranteed to be among the chosen starters. Expected effect in the pre-season window: an attacker with a modest xP gap now takes the armband over a defender/GK whose xP is clean-sheet-heavy, matching what the `captain` command already recommended. Mid-season (penalty auto-off at ≥ 3 live pairs) the ceiling term still breaks near-ties toward goal involvement. |
| v0.27 | 2026-09-03 | **Season-aware rating windows** (calibration change; model_version stays v2, v0.23 pattern). Live-GW residual evidence (26/27, GW1-2): TOT defenders over-predicted −1.8..−5.1 pts/GW while ARS defenders under-predicted +2.9..+7.2 (league-wide DEF calibration fine: −0.11/−0.02) — team-specific rates inherited from last season barely moved between GW1 and GW2 predictions because live rows carry ~2% of the LF38/SF6 blend weight. **Change:** once current-season live rows exist (`player_gw_stats`, v0.23 pipeline), `ratings.compute_team_ratings` / `compute_player_rates` default to **LF20/SF6 blended at 0.6/0.4** (`LIVE_LF_GW_COUNT`/`LIVE_SF_GW_COUNT`/`LIVE_LF_WEIGHT`/`LIVE_SF_WEIGHT`) instead of LF38/SF6 at 0.8/0.2; pre-season (no live rows) is unchanged, and explicit counts/weights still override. League averages use the same in-season params. Doubles live influence at GW2 (~7% → ~13%) and shrinks the long-window lag. **Backtest** (blend simulation, prior 24-25 / live 25-26, no leakage, no change to the pure-prior arm): live-GWs 1-2 MAE 1.667 vs 1.688 baseline (bias −0.040 vs +0.023), live-GWs 3-5 MAE 1.162 vs 1.179 (bias −0.014 vs +0.006), live-GWs 6-8 MAE 1.067 vs 1.069 (bias +0.033 vs +0.020). **Rejected alternative (tested, not shipped):** a team-level live EMA shrinking each team's rates toward its live-season mean with weight n/(n+B) — backtested B∈{2,4,6} and with the new windows: it *adds* error at every bucket (MAE 1.688→1.691/1.689/1.688 at 1-2; 1.179→1.207/1.198/1.194 at 3-5; 1.069→1.110/1.101/1.096 at 6-8; bias worse) because 1-2 live matches are too noisy to shrink toward. Team-specific structural breaks remain an audit-loop (B5) item: residuals decay naturally as live rows dominate the SF window by ~GW6.
