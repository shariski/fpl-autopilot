...
def test_team_ratings_blend_lf_and_sf(db):
    ...
    # Arsenal: xg per gw = 3.5 (2.0+1.5), minutes 180/gw -> xg90 = 1.75
    # Man City: xg 1.0, minutes 180 -> xg90 = 0.5
    assert r[1].xg90 == pytest.approx(1.75) and r[2].xg90 == pytest.approx(0.5)
    ...
    assert la.xg90 == pytest.approx(1.125) and la.xgc90 == pytest.approx(0.75)
    # blended = 0.8*lf + 0.2*sf (all 6 GWs in SF here, so equal)
    assert r[1].xg90 == pytest.approx(0.8 * 1.75 + 0.2 * 1.75)
...
def test_team_ratings_short_form_only_recent_gws(db):
    ...
    assert r[1].xg90 == pytest.approx(1.75)
...
def test_fdr_v2_multipliers_from_ratings(db):
    ...
    # ARS home: opponent MCI xGC90=1.0 / LA 0.75 = 1.333 -> damped 1.333; opponent xG 0.5/1.125=0.444
    assert by[(1, 7)]["fdr_attack_mult"] == pytest.approx(round(ratings.damp(1.0 / 0.75), 4))
    assert by[(1, 7)]["fdr_defense_mult"] == pytest.approx(round(ratings.damp(0.5 / 1.125), 4))
    ...
    assert by[(1, 8)]["fdr_defense_mult"] == pytest.approx(round(ratings.damp(1.72 / 1.125), 4))
